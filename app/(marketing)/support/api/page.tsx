...
export const metadata = {
  ...
  description: "Build on Billabled with scoped API keys for proof packs, revenue intelligence, exports, webhooks, and workspace data.",
};
    ...
    <div className="bg-background text-slate-950">
      ...
      <section className="px-4 py-10 sm:px-6">
        <div className="mx-auto grid max-w-6xl gap-5 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-border bg-surface p-6 shadow-sm shadow-stone-900/5"><LockKeyhole className="h-6 w-6 text-cyan-700" /><h2 className="mt-4 text-xl font-semibold">Protected keys</h2><p className="mt-2 text-sm text-slate-600">Billabled stores a hash, prefix, creator, expiry, revoke status, and last-used timestamp, never the full secret value.</p></div>
      ...
      <section className="px-4 py-8 sm:px-6">
        <div className="mx-auto max-w-6xl rounded-2xl border border-border bg-surface p-6 shadow-sm shadow-stone-900/5">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">Use the Billabled API after the work record is corrected: send proof packs to client systems, pull revenue intelligence into reviews, export work data, and listen for workflow events.</p>
  ...
  --oauth2-bearer "$BILLABLED_API_KEY"`}</code></pre>
      ...
      <section className="px-4 pb-20 pt-8 sm:px-6">
        <div className="mx-auto flex max-w-6xl flex-col gap-5 rounded-2xl bg-slate-950 p-8 text-white shadow-sm md:flex-row md:items-center md:justify-between">
          ...
          <p className="mt-3 text-slate-300">Billing changes, user invites, subscription management, and workspace administration stay inside authenticated Billabled workflows.</p>
          </div>
